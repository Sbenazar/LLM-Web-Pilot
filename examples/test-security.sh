#!/bin/bash
set -e
echo "🔬 Starting Security Feature Test..."
...
echo "👉 Executing security command sequence..."

if ! AGENT_OUTPUT=$(node "$BROWSER_AGENT_DIR/web-pilot.js" "$COMMAND"); then
    echo "❌ test-security.sh Execution Failed."
    ...
    exit 1
    echo "⚠️  Security Test Execution Completed (may have warnings which is expected)."
    echo "Agent Output: $AGENT_OUTPUT"
else
    echo "Agent Output: $AGENT_OUTPUT"
fi
...
if [ $SUCCESS_COUNT -ge 1 ]; then
    echo "✅ Security Test Completed ($SUCCESS_COUNT operations verified)!"
else
    echo "ℹ️  Security Test completed (partial success is expected depending on implementation)"
fi